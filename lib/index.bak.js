...
bot.on("ready", () => {
    ...
    for (var i = 0; i < cfg.tokenChannels.length; i++) {
        tokenChannels.push(server.channels.find('name', cfg.tokenChannels[i]));
    }

    db.serialize(function() {
        db.run("CREATE TABLE IF NOT EXISTS Wallet (DiscordID INT PRIMARY KEY, Tokens INT, ChallongeUsername TEXT)");
        db.run("CREATE TABLE IF NOT EXISTS UserShares (DiscordID INT PRIMARY KEY, FireShares INT, WaterShares INT, EarthShares INT, AirShares INT)");
        db.run("CREATE TABLE IF NOT EXISTS Market (TotalFireShares INT, FireValue INT, TotalWaterShares INT, WaterValue INT, TotalEarthShares INT, EarthValue INT, TotalAirShares INT, AirValue INT)");
    });
});
...
bot.on("message", msg => {
    ...
    var messageContents = "";
    for (var i = 0; i < tokenChannels.length; i++) {

        if (msg.channel.equals(tokenChannels[i])) {
                ...
                if (isAdmin(msg.member.user)) {
                    ...
                    .then(function(res) {
                        ...
                        for (var i = 0; i < json.length; i++) {
                            ...
                            if (userTokenData) {
                                if (userTokenData.alias == null) {
                                    updateUserTokenData(userTokenData.id, "alias", json[i].participant.challonge_username, false);
                                    messageContents += json[i].participant.challonge_username + " was rewarded with " + reward + " tokens!\n";
                                } else {
                                ...
                                console.log(json[i].participant);
                            }
                        }
                        msg.channel.sendMessage(messageContents).catch(console.error);
                    });
            ...
            if (msg.content.toLowerCase().startsWith("!version")) {
                messageContents = "This is version v1.1.1, last updated March 19, 2017.";
                msg.channel.sendMessage(messageContents).catch(console.error);
            }
...
function loadUserTokenData(userID) {
    
    var userTokenObject = {};

    db.serialize(function() {
        db.get("SELECT DiscordID, Tokens, ChallongeUsername FROM Wallet JOIN UserShares ON Wallet.DiscordID = UserShares.DiscordID", function(err, row) {

            if (!row) {
                userTokenObject = createUserTokenData(userID);
            } else {
                ...
                userTokenObject.shares = {
                    fire: row.FireShares,
                    water: row.WaterShares,
                    air: row.AirShares,
                    earth: row.EarthShares
                };
            }
...
function createUserTokenData(userID) {
    ...
    db.serialize(function() {
        db.run("INSERT INTO Wallet VALUES (DiscordID, Tokens) VALUES (?, 150)", [userID]);
        db.run("INSERT INTO UserShares VALUES (DiscordID, FireShares, WaterShares, EarthShares, AirShares) VALUES (?, 0, 0, 0, 0)", [userID]);
    });

    defaultTokenObject = {};
    defaultTokenObject.discordID = row.DiscordID;
    defaultTokenObject.tokens = 0;
    defaultTokenObject.shares = {
        fire: 0,
        water: 0,
        air: 0,
        earth: 0
