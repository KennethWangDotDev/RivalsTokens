...
bot.on('ready', () => {
    ...
    .then(() => {
        ...
        .then(() => {
            var tokenDataFiles = fs.readdirSync(dataPath + '/tokenDataFiles');
            for (var i = 0; i < tokenDataFiles.length; i++) {
                const userTokenData = jsonfile.readFileSync(dataPath + '/tokenDataFiles/' + tokenDataFiles[i]);
                const userShares = userTokenData.shares;
                db.run(`INSERT OR IGNORE INTO UserShares (DiscordID, StockID, Amount) VALUES (${userTokenData.id}, 0, ${userShares["fire"]})`);
                db.run(`INSERT OR IGNORE INTO UserShares (DiscordID, StockID, Amount) VALUES (${userTokenData.id}, 1, ${userShares["water"]})`);
                db.run(`INSERT OR IGNORE INTO UserShares (DiscordID, StockID, Amount) VALUES (${userTokenData.id}, 2, ${userShares["earth"]})`);
                db.run(`INSERT OR IGNORE INTO UserShares (DiscordID, StockID, Amount) VALUES (${userTokenData.id}, 3, ${userShares["air"]})`);
            }
        });
    })
...
bot.on('message', (msg) => {
    ...
    if (command.startsWith('!version')) {
        const messageContent = 'This is version v2.0.1, last updated April 05, 2017.';
        msg.channel.sendMessage(messageContent).catch(console.error);
    }
